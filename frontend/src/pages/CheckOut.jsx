// ✅ Checkout.jsx — includes Redeem Code (Collaboration Coupon) Integration
// 🧠 Added features: Coupon system + 1-time redeem discount + clear comments
...
// 🔹 Component: Recenter Map when user drags marker
...
function CheckOut() {
  ...
  const [couponCode, setCouponCode] = useState("")     // 🆕 Redeem code input
  const [discount, setDiscount] = useState(0)          // 🆕 Discount amount
  const [couponApplied, setCouponApplied] = useState(false) // 🆕 Prevent reuse
  ...
  const AmountWithDeliveryFee = discountedAmount + deliveryFee

  // 🔹 Valid coupon codes (later can come from backend)
  const validCoupons = {
    "COLLAB100": 400,   // For users who donated via Collaboration page
  }
  ...
  const handleApplyCoupon = () => {
    ...
    if (validCoupons[couponCode]) {
      ...
      alert(`Coupon applied! ₹${validCoupons[couponCode]} discount added.`)
      localStorage.removeItem('collabCode') // 🆕 Remove after first use
    } else {
  ...
  // 🔹 Razorpay window for online payments
  ...
  // 🔹 Load user's saved address when component mounts
  useEffect(() => {
    setAddressInput(address)
  }, [address])

  // 🆕 Auto-load coupon from localStorage if came from Collaboration page
  useEffect(() => {
    const storedCoupon = localStorage.getItem('collabCode')
    if (storedCoupon) {
  ...
  return (
    <div className='min-h-screen bg-[#fff9f6] flex items-center justify-center p-6'>
      ...
      <div className='w-full max-w-[900px] bg-white rounded-2xl shadow-xl p-6 space-y-6'>
        ...
        <section>
          <h2 className='text-lg font-semibold mb-3 text-gray-800'>Redeem Collaboration Code</h2>
          <div className='flex gap-2'>
            <input
              type="text"
              className='flex-1 border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#ff4d2d]'
              placeholder='Enter your redeem code (e.g., COLLAB100)'
              value={couponCode}
