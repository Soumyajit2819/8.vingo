...
function DeliveryBoy() {
  ...
  // UPDATED: Added timeout and better error handling
  const sendOtp=async () => {
    setLoading(true)
    setMessage("")
    ...
    try {
      console.log('Sending OTP for order:', currentOrder._id)
  ...
  // UPDATED: Added socket emission after successful OTP verification
    ...
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      <div className='w-full max-w-[800px] flex flex-col gap-5 items-center'>
        ...
        {currentOrder && <div className='bg-white rounded-2xl p-5 shadow-md w-[90%] border border-orange-100'>
          ...
          {/* UPDATED: Better button UI with error message display */}
          {!showOtpBox ? (
            <div>
              <button 
                className='mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-xl shadow-md hover:bg-green-600 active:scale-95 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed' 
                onClick={sendOtp} 
                disabled={loading}
              >
